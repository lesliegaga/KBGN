...
class Data(object):
    def __init__(self, args, path):
        ...
        if( path.find('%') == -1):
            train_origin_file = path + '/train.txt'
        else:
            mark=path.find('%')
            train_origin_file = path[0:mark-3] + '/train.txt'
        ...
        self.train_data_o, self.train_user_dict_o, _ = self._load_ratings(train_origin_file)
        self.test_data, self.test_user_dict, _ = self._load_ratings(test_file)
        ...
        self.kg_data, self.kg_dict, self.relation_dict, self.entity_dict = self._load_kg(kg_file)
    ...
    def _load_ratings(self, file_name):
        ...
        for l in lines:
            ...
            for i_id in pos_ids:
                ...
                item_dict.setdefault(i_id, set())
                item_dict[i_id].add(u_id)
    ...
    def _statistic_ratings(self):
        ...
        self.n_items = max(max(self.train_data_o[:, 1]), max(self.test_data[:, 1])) + 1
        self.n_train = len(self.train_data_o)
    ...
    def _load_kg(self, file_name):
        ...
        kg_dict, relation_dict, entity_dict = _construct_kg(kg_np)

        return kg_np, kg_dict, relation_dict, entity_dict
    ...
    #         return neg_items
    #
    #     def sample_entity_for_i(u, item):
    #         entity_list = []
    #         if item in self.kg_dict:
    #             for entity_info in self.kg_dict[item]:
    #                 eid = entity_info[0] + col_pre
    #                 if u in self.ue_black_dict and eid in self.ue_black_dict[uid]:
    #                     continue
